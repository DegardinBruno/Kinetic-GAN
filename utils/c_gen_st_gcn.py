...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, edge_importance_weighting=True, **kwargs):
        ...
        self.t_size, t_size  = 150, 150

        #kwargs0 = {k: v for k, v in kwargs.items() if k != 'dropout'}
        self.st_gcn_networks = nn.ModuleList((
            st_gcn(in_channels+n_classes, 512, kernel_size, 1, graph=self.graph, lvl=3, residual=False, up_s=False, up_t=int(t_size/16), **kwargs),
    ...
    def forward(self, x, labels):
        ...
        c = c.repeat(1, 1, int(self.t_size/16), 1)
        x = torch.cat((c, x), 1)
        # forward
