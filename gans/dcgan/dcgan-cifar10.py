...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv_blocks = nn.Sequential(
            ...
            nn.ConvTranspose2d(opt.img_size, opt.channels, 4, 2, 1, bias=False),
            nn.Tanh()
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        def discriminator_block(in_filters, out_filters, bn=True):
            block = [nn.Conv2d(in_filters, out_filters, 4, 2, 1, bias=False)]
            if bn:
        ...
        self.model = nn.Sequential(
            *discriminator_block(opt.channels, opt.img_size, bn=False),
            *discriminator_block(opt.img_size, opt.img_size*2),
            *discriminator_block(opt.img_size*2, opt.img_size*4),
            *discriminator_block(opt.img_size*4, opt.img_size*8),
            nn.Conv2d(opt.img_size*8, 1, 4, 1, 0, bias=False),
            nn.Sigmoid()
        )
...
trainset = datasets.CIFAR10(root='/media/socialab/bb715954-b8c5-414e-b2e1-95f4d2ff6f3d', train=True,
                                        download=True, transform=transform)
dataloader = torch.utils.data.DataLoader(trainset, batch_size=opt.batch_size,
