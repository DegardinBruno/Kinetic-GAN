...
body = [trunk_joints, leg_joints, arm_joints]
...
for frame_idx in range(data_numpy.shape[1]):
    ...
    for data in data_numpy:
        ...
        for part in body:
            ...
            y_plot = y[part]
            z_plot = z[part]
            ax.plot(x_plot, y_plot, z_plot, color='#2E477D', marker='o', markersize=7, markerfacecolor='#A7ABB0')
            # ax.plot(x_plot, y_plot, z_plot, color='#9E0B0F', marker='o', markersize=7, markerfacecolor='#bf9494')  # RED


        
    plt.savefig(os.path.join(out,"frame_"+str(frame_idx)+".png"), transparent=True, dpi=600)
    #if frame_idx > 43:
    #plt.show()
