...
root = 'runs/synthetic/videos/3-synthetic-walk-gc-gan-raw'
images = [img for img in humanSort(os.listdir(root)) if img.endswith(".png")]
frame = cv2.imread(os.path.join(root, images[0]))
height, width, layers = frame.shape
print(height, width)
video = cv2.VideoWriter("videos/3-synthetic-walk-gc-gan-raw.mp4", cv2.VideoWriter_fourcc(*'mp4v'),  30 , (width,height))
