...
out        = general.check_runs('dcgan-graph')
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            sample_image(gen_imgs.data[:25], os.path.join(images_out,"%d.npy") % batches_done)
            general.save('dcgan-graph', {'d_loss': loss_d, 'g_loss': loss_g}, 'plot_loss')
